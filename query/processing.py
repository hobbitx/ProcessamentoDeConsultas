...
class QueryRunner:
    ...
    def get_relevance_per_query(self) -> Mapping[str, Set[int]]:
        ...
        for str_file in os.listdir("relevant_docs"):
            ...
            with open(filename) as arq:
                ...
                dic_relevance_docs[arquiv] = set(
                    map(lambda s: str(s).replace("\n", ""), arq.readline().split(","))
                )

        return dic_relevance_docs

    def count_topn_relevant(
        self, n, respostas: List[int], doc_relevantes: Set[int]
    ) -> int:
        """
    ...
    def runQuery(
        query: str,
        indice: Index,
        indice_pre_computado: IndexPreComputedVals,
        map_relevantes,
    ):
        ...
        modelo = int(
            input("Selecione um dos modelos:\n1)Modelo Booleano\n2)Modelo Vetorial")
        )
        if modelo == 1:
        ...
        if query in map_relevantes.keys():
            ...
            precisao = 0
            for n in arr_top:
                revocacao = qr.count_topn_relevant(
                    n, respostas[0], set(map_relevantes[query])
                ) / len(
                    map_relevantes[query]
                )  # substitua aqui pelo calculo da revocacao topN
                precisao = qr.count_topn_relevant(
                    n, respostas[0], set(map_relevantes[query])
                ) / len(
                    respostas[0]
                )  # substitua aqui pelo calculo da revocacao topN
    ...
    def main(index_file: str):       		 
        with open(index_file, "rb") as idx_file:
			print(idx_file)
        	index = FileIndex()        
            t = index.next_from_file(idx_file)
