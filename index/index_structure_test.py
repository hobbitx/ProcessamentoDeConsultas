...
class StructureTest(unittest.TestCase):
    ...
    def test_document_count_with_term(self):
        self.assertEqual(2, self.index.document_count_with_term(
            "casa"), f"Casa apareceu em dois documentos")
        self.assertEqual(3, self.index.document_count_with_term(
            "vermelho"), f"Vemelho apareceu em dois documentos")
        self.assertEqual(1, self.index.document_count_with_term(
            "verde"), f"Verde apareceu em dois documentos")
        self.assertEqual(0, self.index.document_count_with_term(
            "cinza"), f"Cinza não está indexado, deveria retornar zero")
    ...
    def test_get_occurrence_list(self):
        ...
        for term, dic_doc_freq in dict_expected_index.items():
            ...
            for occur in list_occur:
                ...
                self.assertTrue(
                    doc_id in dict_expected_index[term], f"O termo {term} não ocorre no documento {doc_id}")
            ...
            self.assertEqual(len(dic_doc_freq.keys()), len(
                list_occur), f"A lista de occorencia tem ocorrencias a mais (ou a menos) do termo {term}")

        list_occur = self.index.get_occurrence_list('xuxu')
        self.assertListEqual(
            list_occur, [], "O termo xuxu não existe, deveria retornar lista vazia")
