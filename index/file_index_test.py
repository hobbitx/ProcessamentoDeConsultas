...
class FileIndexTest(unittest.TestCase):
    ...
    def check_idx_file(self, obj_index, set_occurrences):
        ...
        with open(obj_index.str_idx_file_name, "rb") as idx_file:
            ...
            while occur is not None:
                self.assertTrue(
                    occur > last_occur, msg=f"A ocorrencia {last_occur} foi colocada de forma incorreta antes da ocorrencia {occur}")
        ...
        self.assertEqual(len(
            sobra_arquivo), 0, f"Existem ocorrências no arquivo que não estavam na 'lst_occurrences_tmp': {sobra_arquivo} ")
        self.assertEqual(len(
            sobra_lista), 0, f"As seguintes ocorrências não foram inseridas no arquivo de indice: {sobra_lista} ")
    ...
    def test_finish_indexing(self):
        ...
        [self.assertEqual(self.index.dic_index[arr_termos[i]].term_id, i+1,
                          f"O id do termo {i+1} mudou para {self.index.dic_index[arr_termos[i]].term_id}") for i in range(4)]

        # testa se a quantidade de documentos que possuem um determinado termo está correto
        arr_pos_por_termo = [0, int_size_of_occur*3,
                             int_size_of_occur*6, int_size_of_occur*7]
        arr_pos = [1, 4, 7, 8]
        [self.assertEqual(self.index.dic_index[arr_termos[i]].term_file_start_pos, arr_pos_por_termo[i],
                          f"A posição inicial do termo de id {i+1} no arquivo seria {arr_pos_por_termo[i]} (ou seja, antes da {arr_pos[i]}ª ocorrencia) e não {self.index.dic_index[arr_termos[i]].term_file_start_pos}") for i in range(4)]

        # testa se a quantidade de documentos que possuem um determinado termo está correto
        arr_doc_por_termo = [3, 3, 1, 2]
        [self.assertEqual(self.index.dic_index[arr_termos[i]].doc_count_with_term, arr_doc_por_termo[i],
                          f"A quantidade de documentos que possuem o termo de id {self.index.dic_index[arr_termos[i]].term_id} seria {arr_doc_por_termo[i]} e não {self.index.dic_index[arr_termos[i]].doc_count_with_term}") for i in range(4)]
