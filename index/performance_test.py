...
class PerformanceTest(unittest.TestCase):
    ...
    def print_status(self, count: int, total: int):
        ...
        print(
            f"Memoria usada: {current / 10**6:,} MB; Máximo {peak / 10**6:,} MB", flush=True)
        print(
            f"Indexando ocorrencia #{count:,}/{total:,} ({porc_complete}%)", flush=True)
    ...
    def index_words(self):
        ...
        for doc_i in range(PerformanceTest.NUM_DOCS):
            for term_j in range(PerformanceTest.NUM_TERM_PER_DOC):
                ...
                if count % 50000 == 0:
                    self.print_status(
                        count, PerformanceTest.NUM_DOCS*PerformanceTest.NUM_TERM_PER_DOC)
